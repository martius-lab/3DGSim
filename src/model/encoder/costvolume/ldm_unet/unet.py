...
from .util import avg_pool_nd, checkpoint, conv_nd, linear, normalization, timestep_embedding, zero_module
...
class StackUNet(nn.Module):
    def __init__(
        ...
        for i in range(num_stacks - 1):
            self.convs.append(
                zero_module(conv_nd(dims, out_channels, in_channels, 3, padding=1))
            )
