...
def render_cuda(
    ...
    gaussian_opacities: Float[Tensor, "batch gaussian 1"],
    gaussians_masks: Optional[Bool[Tensor, "batch gaussian 1"]] = None,
    ...
    for i in range(b):
        ...
        if gaussians_masks is not None:
            mask_i = gaussians_masks[i].squeeze(dim=-1)
            rasterizer_kwargs_i = {k: v[mask_i] for k, v in rasterizer_kwargs_i.items() if v is not None}

        image, radii = rasterizer(**rasterizer_kwargs_i)
